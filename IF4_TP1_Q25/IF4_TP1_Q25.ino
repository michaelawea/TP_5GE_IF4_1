...
void vTask1( void *pvParameters )
{
  ...
  Serial.printf("[INFO] task1 / core %d started\n", xPortGetCoreID());
  ...
  for (;;) {
    ...
    if( status == pdPASS ) {
      Serial.printf("[INFO] task1: a envoyé %d\n", valueToSend);
      alternate(oscillo1, half_period_ms);    
    }
    else
      Serial.printf("[INFO] task1: PAS PU ENVOYER %d\n",  valueToSend);
...
void vTask2( void *pvParameters )
{
  ...
  Serial.printf("[INFO] task2 / core %d started\n", xPortGetCoreID());
  
  // boucle infinie
  for (;;) {

    Serial.printf("[INFO] task2: %d données en attente\n", uxQueueMessagesWaiting( xQueue ));

    status = xQueueReceive( xQueue, &receivedValue, pdMS_TO_TICKS( 100 ) );
    if( status == pdPASS ) {
      Serial.printf("[INFO] task2: reçu %d\n", receivedValue);
      // simule du boulot     
      alternate(oscillo2, half_period_ms); 
    }
    else
      Serial.printf("[INFO] task2: RIEN EU A LIRE\n");
